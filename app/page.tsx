  ...
  return (
    <div className="flex h-[100dvh] flex-col overflow-hidden bg-background text-foreground">
      ...
      <main className="flex flex-1 min-h-0 flex-col items-center justify-center px-4 text-center sm:px-10">
        <div className="mb-4 inline-flex max-w-full items-center gap-2 whitespace-nowrap rounded-full border border-highlight/40 bg-highlight/10 px-3 py-1.5 text-[10px] font-medium uppercase tracking-wide text-highlight sm:mb-5 sm:px-4 sm:text-xs sm:tracking-[0.15em]">
          <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-highlight" />
          <span>
            Summer Enrollment Open
            <span className="hidden min-[400px]:inline">
              {" "}
              &middot; Ages 8&ndash;18
            </span>
          </span>
        </div>

        <h1 className="max-w-3xl text-balance font-serif text-3xl font-medium leading-[1.15] tracking-tight sm:text-5xl md:text-6xl">
          This summer, they build &mdash; not just scroll.
        </h1>

        <p className="mx-auto mt-3 max-w-xl text-balance text-sm leading-relaxed text-muted sm:mt-5 sm:text-lg">
      ...
      <section className="shrink-0 border-t border-border">
        <div className="mx-auto grid max-w-4xl grid-cols-2 sm:grid-cols-4">
          {stats.map((stat, i) => (
            <div
              key={stat.label}
              className={`flex flex-col items-center gap-0.5 px-2 py-2.5 text-center sm:gap-1 sm:px-4 sm:py-5 ${
                i % 2 === 0 ? "border-r border-border" : ""
              } ${i < 2 ? "border-b border-border sm:border-b-0" : ""} ${
                i > 0 ? "sm:border-l sm:border-r-0" : ""
              }`}
            >
              <span className="font-serif text-base text-accent sm:text-2xl">
                {stat.value}
              </span>
              <span className="text-[9px] uppercase tracking-[0.08em] text-muted sm:text-[11px] sm:tracking-[0.1em]">
                {stat.label}
              </span>
            </div>
          ))}
      ...
      {/* Footer */}
      <footer className="flex shrink-0 flex-col items-center gap-1 border-t border-border px-4 py-2.5 text-center text-[10px] text-muted sm:flex-row sm:justify-between sm:gap-2 sm:px-10 sm:py-4 sm:text-left sm:text-xs">
        <span>
          &copy; 2026 Code Illustrated Online School &middot;{" "}
          <a
            href={SCHOOL_URL}
